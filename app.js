  ...
  function giveFeedback(guess) {
    if (checkInput(parseInt(guess))) {
      if (history.includes(guess)) {
        feedbackP.innerHTML = `Hey! You already guessed <span>${guess}</span>.`;
      } else {
      ...
      feedbackP.textContent =
        "Ummm, no. The rules are to enter a whole number between 1 and 100, silly.";
    }
  ...
  function checkCorrectAnswer(guess) {
    ...
    if (guess > randomNumber) {
      feedbackP.textContent = `${guess} is too high. Give it another shot!`;
    } else if (guess < randomNumber) {
      feedbackP.textContent = `${guess} is too low. Guess again!`;
    } else {
      playing = false;
      feedbackP.textContent = `You got it! ${guess} is the correct number!`;
      input.disabled = true;
      if (highscore) {
